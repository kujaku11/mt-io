...
class LEMI417:
    ...
    def run_metadata(self) -> Run:
        ...
        if self._has_data() and not self.header_df.empty:
            r.data_logger.power_source.voltage.start = float(
                self.header_df["vin"].max()
            )
    ...
    def read(self, fn: Optional[Union[str, Path]] = None) -> LEMI417:
        ...
        with open(self.fn, "rb") as f:
            ...
            for iblock in range(nblock):
                ...
                elev = (
                    int(self._bcd_to_int(tag[23])) * 1000
                    + int(self._bcd_to_int(tag[24])) * 10
                )
                ...
                raw = (
                    np.frombuffer(raw_data, dtype=np.uint8).reshape(16, 30).astype(int)
                )
